/* This file was generated by SableCC (http://www.sablecc.org/). */

package compiladorCMenos.node;

import compiladorCMenos.analysis.*;

@SuppressWarnings("nls")
public final class AUnicoSeqDeclParametro extends PSeqDeclParametro
{
    private PDeclParametro _param_;

    public AUnicoSeqDeclParametro()
    {
        // Constructor
    }

    public AUnicoSeqDeclParametro(
        @SuppressWarnings("hiding") PDeclParametro _param_)
    {
        // Constructor
        setParam(_param_);

    }

    @Override
    public Object clone()
    {
        return new AUnicoSeqDeclParametro(
            cloneNode(this._param_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAUnicoSeqDeclParametro(this);
    }

    public PDeclParametro getParam()
    {
        return this._param_;
    }

    public void setParam(PDeclParametro node)
    {
        if(this._param_ != null)
        {
            this._param_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._param_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._param_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._param_ == child)
        {
            this._param_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._param_ == oldChild)
        {
            setParam((PDeclParametro) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
